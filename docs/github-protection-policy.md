...
for review, not approval by itself. In public-strict or ruleset-backed modes,
enforce that process gate with a required approving review. Merging, release
decisions, rollback of policy-sensitive changes, and branch-protection changes
still require an explicit human approval path.

When rulesets are available, prefer rulesets over branch protection for the long-term configuration:

- `main` branch ruleset: pull request required, `check` required, up-to-date required, linear history required, force pushes blocked, deletion blocked, CODEOWNER review required for public-strict operation.
...
public-strictまたはruleset-backed modeでは、そのprocess gateをapproving review必須化で
強制します。merge、release判断、policy-sensitive changeのrollback、branch protection変更には
明示的なhuman approval経路が必要です。

rulesets が使える場合は、長期設定として branch protection より rulesets を優先します。

- `main` branch ruleset: pull request必須、`check`必須、up-to-date必須、linear history必須、force push禁止、deletion禁止、public-strict運用ではCODEOWNER review必須。
