...
- Release artifacts for `v0.1.2` and later are signed with Sigstore keyless
...
`v0.1.1-baseline` records migrated baseline provenance. For `v0.1.2` and
later, the release workflow publishes a source archive, SHA256 checksum, and
Sigstore bundle. Do not mark OpenSSF signed-release criteria as met for a
release until the release artifacts are published and verified with the
documented `cosign verify-blob` command.
